...
fetch('../API/login_check.php')
    ...
    .then(data => {
        if (!data.success) {
            ...
            cartcount.setAttribute("data-count", cart.length); // Set the cart count
...
async function searchProduct(input) {
    ...
    if(matchedProducts != null) {
        ...
        limit = 5; // Reset limit for new search
        currentProducts = matchedProducts.slice(offset, offset + limit); // Get the first set of products
        displayProducts();
    }else console.log('No matching products');
}
...
function displayProducts() {
    ...
    function handlePreviousClick() {
        if (offset >= limit) {
            ...
            currentProducts = matchedProducts.slice(offset, offset + limit); // Get the previous set of products
    ...
    function handleNextClick() {
        if (offset + limit < matchedProducts.length) {
            ...
            currentProducts = matchedProducts.slice(offset, offset + limit); // Get the next set of products
            displayBestSellers();  // Display updated products
        }
    }
